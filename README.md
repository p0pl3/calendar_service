...
4. При создании напоминания Event Service вызывает `GET /users/me` на User Service (пробрасывая токен пользователя), чтобы получить email — он нужен для последующей отправки уведомления.
...
3. **Notification Service** подписан на очередь `reminder_tasks` через aio-pika. При получении сообщения отправляет письмо через SMTP (aiosmtplib).
4. Ошибки (SMTP недоступен) логируются, сообщение подтверждается (ack) — очередь не блокируется.
5. При потере соединения с RabbitMQ — автоматически переподключается (цикл с `try/except`, `sleep(5)`).
...
- **Профиль** (`GET/PUT/DELETE /users/me`): возвращает/обновляет данные текущего пользователя.
...
- **Авторизация**: JWT декодируется локально через тот же `SECRET_KEY` — нет сетевого вызова к User Service при каждом запросе.

Зависимости: **PostgreSQL** (хранение), **RabbitMQ** (публикация задач), **User Service** (за email при создании напоминания).
...
- Получает сообщение → десериализует JSON → отправляет email через aiosmtplib.
- Ошибки логируются, сообщение подтверждается (ack) — очередь не блокируется.

Зависимости: **RabbitMQ** (получение задач), **SMTP** (email).
...
|---|---|---|
| `users` | User Service | Профили, хэши паролей |
...
| **Notification Service** | `$HOST_PORT_NOTIFY_SVC` (8003) | FastAPI, aio-pika, aiosmtplib |
...
cp .env.example .env
# Отредактировать .env: SMTP_USER, SMTP_PASSWORD
...
curl -X POST http://localhost/reminders/ \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"event_id":"<id события>","remind_at":"2025-06-01T09:58:00Z","channels":["email"]}'
```
...
practice2/
  ...
  tests/
    user-service/         25 тестов
    event-service/        24 теста
    notification-service/ 9 тестов
```
